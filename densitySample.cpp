...
DensitySample::DensitySample(const vector<vector<real> > & w,
...
void DensitySample::precomputeFactors(const Dataset & S, vector<real> & factors,
...
/*
real DensitySample::evalS(const Datapoint & X) const {
  real t = 0.;
  for (int j = 0; j < phi.size(); ++j) {
...
real DensitySample::evalS(int i) const {
  return expFactorsS[i] / normalizerS;
}
*/

real DensitySample::eval(const Datapoint & X) const {
  real t = 0.;
  for (int j = 0; j < phi.size(); ++j) {
    vector<real> phix = phi[j]->eval(X);
    for (int k = 0; k < phix.size(); ++k)
...
real DensitySample::loss(real beta) const {
  real loss = 0.;
...
vector<real> DensitySample::EphiPW(int j) const {
  // TODO this should be precomputed at creation of pw?
  const Feature* thisphi = phi[j];
  vector<real> E(thisphi->size(), 0.);
  for (int i = 0; i < Sp.size(); ++i) {
    real pwsi = expFactorsSp[i] / normalizerSp;
    vector<real> phix = thisphi->eval(Sp[i]);
    for (int j = 0; j < thisphi->size(); ++j)
      E[j] += pwsi * phix[j];
  }
  return E;
}
